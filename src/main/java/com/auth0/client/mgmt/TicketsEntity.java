...
public class TicketsEntity extends BaseManagementEntity {
    ...
    public Request<EmailVerificationTicket> requestEmailVerification(EmailVerificationTicket emailVerificationTicket) {
        ...
        ExtendedBaseRequest<EmailVerificationTicket> request = new ExtendedBaseRequest<>(client, url, HttpMethod.POST, new TypeReference<EmailVerificationTicket>() {
        });
    ...
    public Request<PasswordChangeTicket> requestPasswordChange(PasswordChangeTicket passwordChangeTicket) {
        ...
        ExtendedBaseRequest<PasswordChangeTicket> request = new ExtendedBaseRequest<>(client, url, HttpMethod.POST, new TypeReference<PasswordChangeTicket>() {
        });
        request.addHeader("Authorization", "Bearer " + apiToken);
