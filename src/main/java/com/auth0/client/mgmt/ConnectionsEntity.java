...
public class ConnectionsEntity extends BaseManagementEntity {
    ...
    public Request<ConnectionsPage> listAll(ConnectionFilter filter) {
        ...
        CustomRequest<ConnectionsPage> request = new CustomRequest<>(client, tokenProvider, url, HttpMethod.GET, new TypeReference<ConnectionsPage>() {
        });
    ...
    public Request<List<Connection>> list(ConnectionFilter filter) {
        ...
        String url = builder.build().toString();
        CustomRequest<List<Connection>> request = new CustomRequest<>(client, tokenProvider, url, HttpMethod.GET, new TypeReference<List<Connection>>() {
        });
        // // request.addHeader("Authorization", "Bearer " + apiToken);
        return request;
    }
    ...
    public Request<Connection> get(String connectionId, ConnectionFilter filter) {
        ...
        String url = builder.build().toString();
        CustomRequest<Connection> request = new CustomRequest<>(client, tokenProvider, url, HttpMethod.GET, new TypeReference<Connection>() {
        });
        // // request.addHeader("Authorization", "Bearer " + apiToken);
        return request;
    }
