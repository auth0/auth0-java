...
public class ClientGrantsEntity extends BaseManagementEntity {
    ...
    public Request<ClientGrantsPage> list(ClientGrantsFilter filter) {
        ...
        CustomRequest<ClientGrantsPage> request = new CustomRequest<>(client, tokenProvider, url, HttpMethod.GET, new TypeReference<ClientGrantsPage>() {
        });
    ...
    public Request<List<ClientGrant>> list() {
        String url = baseUrl
                ...
                .build()
                .toString();
        CustomRequest<List<ClientGrant>> request = new CustomRequest<>(client, tokenProvider, url, HttpMethod.GET, new TypeReference<List<ClientGrant>>() {
        });
        // // request.addHeader("Authorization", "Bearer " + apiToken);
        return request;
    }
