...
public class GrantsEntity extends BaseManagementEntity {
    ...
    public Request<GrantsPage> list(String userId, GrantsFilter filter) {
        ...
        ExtendedBaseRequest<GrantsPage> request = new ExtendedBaseRequest<>(client, url, HttpMethod.GET, new TypeReference<GrantsPage>() {
        });
    ...
    public Request<List<Grant>> list(String userId) {
        ...
        String url = baseUrl
                ...
                .build()
                .toString();
        ExtendedBaseRequest<List<Grant>> request = new ExtendedBaseRequest<>(client, url, HttpMethod.GET, new TypeReference<List<Grant>>() {
        });
        request.addHeader("Authorization", "Bearer " + apiToken);
        return request;
    }
